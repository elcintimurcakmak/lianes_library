...
if choice == "Welcome":
    ...
    st.markdown(f"""<style>.stApp {{{bg_style} background-size: cover;}}</style>""", unsafe_allow_html=True)
...
# ---------------- DASHBOARD ----------------
    ...
    if not books.empty:
        ...
        df = df.merge(friends, left_on="friend_id", right_on="id", how="left", suffixes=("", "_f"))
...
# ---------------- RETURN ----------------
    ...
    if not active.empty:
        lid = st.selectbox("Loan", active["id"])
